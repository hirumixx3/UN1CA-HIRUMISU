...
LOG "- All partition images were generated"

PRESERVE_DIR="$OUT_DIR/a05s-oneui9-images"
...
GENERATED_FILES=(
    "$TMP_DIR"/*.img
    "$TMP_DIR"/*.map
    "$TMP_DIR/build_info.txt"
)

if [ "${#GENERATED_FILES[@]}" -eq 0 ]; then
    LOGE "No generated images found in $TMP_DIR"
    exit 1
fi

mv -f "${GENERATED_FILES[@]}" "$PRESERVE_DIR/"

shopt -u nullglob

LOG "- Images moved permanently to: $PRESERVE_DIR"
exit 0
